...
import { Search, ChevronDown, ChevronUp, User, Mail, Phone, Briefcase, Calendar, Store, MapPinHouse, ArchiveX, ArchiveRestore } from 'lucide-react';
...
interface VendorTableProps {
  ...
  vendorType: string;
  isLoading: boolean;
  data: InfiniteData<VendorsPage> | undefined;
  isError: boolean;
...
const VendorTable: React.FC<VendorTableProps> = ({ isLoading, data, vendorType, isFetchingNextPage, hasNextPage, fetchNextPage, isError, vendors }) => {
  const [filteredData, setFilteredData] = useState<VendorDataType[] | any>([]);

  const [sortField, setSortField] = useState<keyof VendorDataType | null>(null);
  ...
  // Sort handler
  const handleSort = (field: keyof VendorDataType) => {
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-slate-700/50">
                          {[
                            { key: 'username' as keyof VendorDataType, label: 'Username', icon: User },
                            { key: 'email' as keyof VendorDataType, label: 'Email', icon: Mail },
                            { key: 'phone' as keyof VendorDataType, label: 'Phone', icon: Phone },
                            { key: 'status' as keyof VendorDataType, label: 'Status', icon: Briefcase },
                            { key: 'shop_name' as keyof VendorDataType, label: 'Shop Name', icon: Store },
                            { key: 'shop_city' as keyof VendorDataType, label: 'City', icon: MapPinHouse },
                            { key: 'oreders' as keyof VendorDataType, label: 'Totle Orders', icon: ArchiveRestore },
                            { key: 'rejected_orderds' as keyof VendorDataType, label: 'Reject Orders', icon: ArchiveX },
                            { key: 'createAt' as keyof VendorDataType, label: 'Join Date', icon: Calendar },
                      ...
                      <tbody>
                        {isLoading && filteredData.length === 0 ? (
                          ...
                          <AnimatePresence mode="popLayout">
                            {filteredData?.map((item: VendorDataType, index: number) => (
                              <motion.tr
                                key={item?.id}
                              ...
                              >
                                ...
                                {item?.status && <td className="px-6 py-4">
                                  <span className="px-3 py-1 text-xs font-medium bg-blue-500/20 text-blue-300 rounded-full">
                                    {item?.status}
                                  </span>
                                </td>}
                                <td className="px-6 py-4 text-sm text-slate-300">{item?.shop_name}</td>
                                <td className="px-6 py-4 text-sm text-slate-300">{item?.shop_city}</td>
                                <td className="px-6 py-4 text-sm text-slate-300">{item?.oreders || 0}</td>
                                <td className="px-6 py-4 text-sm text-slate-300">{item?.rejected_orderds || 0}</td>
