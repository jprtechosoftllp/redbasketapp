...
import { useAppSelector } from 'apps/manager-ui/src/utils/redux/store/hook';
import { setVendor } from 'apps/manager-ui/src/utils/redux/slice/vendorSlice';
import { getVendor } from 'apps/manager-ui/src/utils/actions/manager';
...
const ManagerDashboard = () => {
  ...
  const managers = useAppSelector((state) => state.vendors.list);
  ...
  const {
    ...
    queryFn: ({ pageParam = 1 }) => getVendor({ pageParam }),
  ...
  useEffect(() => {
    if (data?.pages) {
      const allManagers = data.pages.flatMap((page) => page.managers);
      if (allManagers.length > 0) {
        dispatch(setVendor(allManagers));
      }
    }
  }, [data, dispatch]);
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-700/50">
                    {[
                      { key: 'name' as keyof VendorDataType, label: 'Name', icon: User },
                      { key: 'email' as keyof VendorDataType, label: 'Email', icon: Mail },
                      { key: 'phone' as keyof VendorDataType, label: 'Phone', icon: Phone },
                      { key: 'role' as keyof VendorDataType, label: 'Role', icon: Briefcase },
